...
POST_PROCESSING = "clean"  # "none" | "clean" | "markdown"

# 🏷️ Κατάληξη του τοπικού αρχείου ανά mode, ίδια με αυτή που δίνει η υπηρεσία στο object.
OUTPUT_SUFFIXES = {"none": ".txt", "clean": "_clean.txt", "markdown": ".md"}

# 🎨 Εικονίδιο ανά mode, μόνο για το output.
MODE_ICONS = {"none": "📝", "clean": "🧼", "markdown": "📑"}
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="🎛️ Τρόπος επεξεργασίας του κειμένου μετά την εξαγωγή.",
    )
...
def main() -> int:
    ...
    print("📄 diavgeia_parsing — εξαγωγή κειμένου από PDF μέσω MinIO 📦\n")

    # 1️⃣ Βεβαιώσου ότι η υπηρεσία και το MinIO είναι όρθια.
    check_service(args.base_url)

    # 2️⃣ Βρες όλα τα PDFs.
    pdfs = find_pdfs(args.pdf_dir)
    if not pdfs:
        print(f"🤷 Δεν βρέθηκε κανένα PDF στο {args.pdf_dir}.")
        return 1

    mode_icon = MODE_ICONS.get(args.post_processing, "📝")
    print(f"📂 Βρέθηκαν {len(pdfs)} PDF αρχεία {mode_icon} (post_processing={args.post_processing})\n")

    # 3️⃣ Ανέβασε, εξήγαγε και κατέβασε το καθένα.
    failures = 0
    ...
    # 4️⃣ Σύνοψη.
    ok = len(pdfs) - failures
    icon = "🎉" if failures == 0 else "⚠️"
    print(f"\n{icon} Ολοκληρώθηκαν {ok}/{len(pdfs)} — έξοδος στο {args.out_dir}/ 📁")
